...
function(_iplug_create_desktop_targets plugin_name formats sources ui_lib resources web_resources base_lib)
  ...
  # On Windows, embed resources/main.rc (fonts, images, icon, etc.) into every
  # desktop target, not just APP. Plugin format targets are MODULE libraries
  # built from `sources` alone, so without the .rc file they contain no Win32
  # resources at all: IGraphics::LoadBitmap/LoadFont then silently fail to
  # find anything (Skia) or hard-assert "Bitmap not found" (NanoVG - e.g.
  # forced on Windows ARM64EC, where Skia isn't supported).
  set(_module_sources ${sources})
  if(WIN32)
    set(_rc_file "${CMAKE_CURRENT_SOURCE_DIR}/resources/main.rc")
    if(EXISTS "${_rc_file}")
      list(APPEND _module_sources "${_rc_file}")
  ...
  if("APP" IN_LIST formats)
    add_executable(${plugin_name}-app ${_module_sources})
  ...
  if("VST2" IN_LIST formats AND IPLUG2_VST2_SUPPORTED)
    add_library(${plugin_name}-vst2 MODULE ${_module_sources})
  ...
  if("VST3" IN_LIST formats)
    add_library(${plugin_name}-vst3 MODULE ${_module_sources})
  ...
  if("CLAP" IN_LIST formats AND IPLUG2_CLAP_SUPPORTED)
    add_library(${plugin_name}-clap MODULE ${_module_sources})
  ...
  if("AAX" IN_LIST formats AND IPLUG2_AAX_SUPPORTED)
    add_library(${plugin_name}-aax MODULE ${_module_sources})
