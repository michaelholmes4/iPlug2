...
static int glnvg__renderCreate(void* uptr)
{
	...
	static const char* fillFragShader =
		...
		"#endif\n"
		"\n"
		"// High quality dithering for gradients\n"
		"float my_noise(vec2 uv_coord) {\n"
		"	float noise = fract(sin(dot(uv_coord, vec2(12.9898, 78.233))) * 43758.5453);\n"
		"	return (1.0 / 255.0) * noise - (0.5 / 255.0);\n"
		"}\n"
		"\n"
		"float fastsin(float x) {\n"
		"	float norm = fract(x * 1.0 / 3.14159);\n"
		"	norm = x > 0.0 ? norm : 1.0 - norm;\n"
		"	float y = -norm * abs(norm) + norm;\n"
		"	return 4.0 * y;\n"
		"}\n"
		"\n"
		"float my_noise_optimised(vec2 uv_coord) {\n"
		...
		"		float noise = my_noise_optimised(fpos);\n"
		"		// Combine alpha\n"
		"		color *= strokeAlpha;\n"
		"		color.rgb += noise;\n"
		"		color *= scissor;\n"
